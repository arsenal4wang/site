package com.Bean;

public class News {

	int newsID, readTimes, secSecID, secFirstID,haveFile;
	String newsTitle, newsContent, author, subDate, modAuthor, modDate,
			secSecName, isChecked, secFirstName, isTop, titleFake, latest;// titleFake如果过长，只显示一部分


	public int getHaveFile() {
		return haveFile;
	}

	public void setHaveFile(int haveFile) {
		this.haveFile = haveFile;
	}

	public String getLatest() {
		return latest;
	}

	public void setLatest(String latest) {
		this.latest = latest;
	}

	public String getTitleFake() {
		return titleFake;
	}

	public void setTitleFake(String titleFake) {
		this.titleFake = titleFake;
	}

	public int getNewsID() {
		return newsID;
	}

	public void setNewsID(int newsID) {
		this.newsID = newsID;
	}

	public int getReadTimes() {
		return readTimes;
	}

	public void setReadTimes(int readTimes) {
		this.readTimes = readTimes;
	}

	public int getSecSecID() {
		return secSecID;
	}

	public void setSecSecID(int secSecID) {
		this.secSecID = secSecID;
	}

	public int getSecFirstID() {
		return secFirstID;
	}

	public void setSecFirstID(int secFirstID) {
		this.secFirstID = secFirstID;
	}

	public String getNewsTitle() {
		return newsTitle;
	}

	public void setNewsTitle(String newsTitle) {
		this.newsTitle = newsTitle;
	}

	public String getNewsContent() {
		return newsContent;
	}

	public void setNewsContent(String newsContent) {
		this.newsContent = newsContent;
	}

	public String getAuthor() {
		return author;
	}

	public void setAuthor(String author) {
		this.author = author;
	}

	public String getSubDate() {
		return subDate;
	}

	public void setSubDate(String subDate) {
		this.subDate = subDate;
	}

	public String getModAuthor() {
		return modAuthor;
	}

	public void setModAuthor(String modAuthor) {
		this.modAuthor = modAuthor;
	}

	public String getModDate() {
		return modDate;
	}

	public void setModDate(String modDate) {
		this.modDate = modDate;
	}

	public String getSecSecName() {
		return secSecName;
	}

	public void setSecSecName(String secSecName) {
		this.secSecName = secSecName;
	}

	public String getIsChecked() {
		return isChecked;
	}

	public void setIsChecked(String isChecked) {
		this.isChecked = isChecked;
	}

	public String getSecFirstName() {
		return secFirstName;
	}

	public void setSecFirstName(String secFirstName) {
		this.secFirstName = secFirstName;
	}

	public String getIsTop() {
		return isTop;
	}

	public void setIsTop(String isTop) {
		this.isTop = isTop;
	}

}